...
class Solo12InvKin:
    def __init__(self):
        ...
        #Inputs
        self.dt = 0.001
        self.feet_position_ref =     [np.array([0.1946,   0.16891, 0.0191028]),np.array([0.1946,  -0.16891, 0.0191028]),np.array([-0.1946,   0.16891, 0.0191028]),np.array([-0.1946,  -0.16891, 0.1191028])]
        ...
        self.flag_in_contact = np.array([1,0,0,0])
        ...
        self.feet_position_ref = [
            lambda t: 0*np.array([0.1946,   0.16891, 0.0191028]),
            lambda t: 0.0*np.array([0.1946,  -0.16891, 0.0191028]),
            lambda t: np.array([-0.1946,   0.16891, (1-np.cos(10*t))*.1+0.0191028]),
            #np.array([-0.1946,   0.16891, 0.1191028]),
            lambda t: np.array([-0.1946,  -0.16891, 0.1191028])
        ]
        self.feet_velocity_ref = [
            lambda t: np.array([0,0,0.]),
            lambda t: np.array([0,0,0.]),
            lambda t: np.array([0,0, np.sin(10*t)]),
            lambda t: np.array([0,0,0.]),
        ]
        self.feet_acceleration_ref = [
            lambda t: np.array([0,0,0.]),
            lambda t: np.array([0,0,0.]),
            lambda t: np.array([0,0, 10*np.cos(10*t)]),
            lambda t: np.array([0,0,0.]),
        ]
    ...
    def compute(self,q,dq):
        ...
        for i_ee in range(4):
            ...
            ref = self.feet_position_ref[i_ee](self.i*self.dt)
            vref = self.feet_velocity_ref[i_ee](self.i*self.dt)
            aref = self.feet_acceleration_ref[i_ee](self.i*self.dt)
            
            J1 = pin.computeFrameJacobian(self.robot.model,self.robot.data,q,idx,pin.LOCAL_WORLD_ALIGNED)[:3]
            acc1 = -self.Kp_flyingfeet*(pos-ref) - Kd_flyingfeet*(nu.linear-vref) + aref
